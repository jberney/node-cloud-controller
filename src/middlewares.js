...
module.exports = ({
  new: ({info, knexHelper}) => ({
    ...
    listAll: async ({params: {from}, query}, res) => {
      let {
        page = 1, 'results-per-page': perPage = 100, 'order-by': orderBy = 'id', 'order-direction': orderDir = 'asc'
      } = query;
      ...
      const count = await knexHelper.tableCount(from);
      ...
      await knexHelper.streamPage({from, page, perPage, orderBy, orderDir, write});
