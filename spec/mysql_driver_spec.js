  ...
  describe('writeRows', () => {
    ...
    describe('page = 1', () => {
      ...
      it('queries the database', () => {
        expect(connection.query).toHaveBeenCalledWith({
          sql: `SELECT \`organizations\`.\`name\`,\`organizations\`.\`billing_enabled\`,\`quota_definitions\`.\`guid\`,\`organizations\`.\`status\`,\`organizations\`.\`guid\` FROM \`${from}\` LEFT JOIN \`quota_definitions\` ON \`${from}\`.\`quota_definition_id\` = \`quota_definitions\`.\`id\` ORDER BY \`id\` ASC LIMIT 100`,
          nestTables: true
        });
      });
    ...
    describe('page > 1', () => {
      ...
      beforeEach(() => {
        ...
        connection.query.and.callFake(({sql}, cb) => {
          if (sql.indexOf('OFFSET') !== -1) return cb(null, [{id: 99}]);
          resolve();
          return query;
        });
        MySqlDriver.writeRows({connection, entity, from, res, page: 2});
      });

      it('finds the previous id', () => {
        const sql = `SELECT \`organizations\`.\`id\` FROM \`${from}\` LEFT JOIN \`quota_definitions\` ON \`${from}\`.\`quota_definition_id\` = \`quota_definitions\`.\`id\` ORDER BY \`id\` ASC LIMIT 1 OFFSET 99`;
        expect(connection.query).toHaveBeenCalledWith({sql}, jasmine.any(Function));
      });

      it.async('queries the database', async () => {
        await promise;
        expect(connection.query).toHaveBeenCalledWith({
          sql: `SELECT \`organizations\`.\`name\`,\`organizations\`.\`billing_enabled\`,\`quota_definitions\`.\`guid\`,\`organizations\`.\`status\`,\`organizations\`.\`guid\` FROM \`${from}\` LEFT JOIN \`quota_definitions\` ON \`${from}\`.\`quota_definition_id\` = \`quota_definitions\`.\`id\` WHERE \`organizations\`.\`id\` > 99 ORDER BY \`id\` ASC LIMIT 100`,
          nestTables: true
        });
      });
    });
